...
const selectLayer = (layer, info, mode) => {
    ...
    if(name.length>max_len){
        document.getElementById('NodeName').innerText = name.slice(0, max_len-1)+"..."
    }else{
        document.getElementById('NodeName').innerText = name
    }
    
    document.getElementById('NodeName').title = name
    
    document.getElementById('NodeConfig').innerHTML = config
}
...
const getAttr = (attr) => {
    if (Object.keys(val)[0] == "val"){
        ...
        else if (Object.keys(val["list"])[0] == "tensor"){
            return "tensor"
        }
        else {
            return val["list"][Object.keys(val["list"])[0]]
        }
...
// get layer attr
const layerAttr = (layer, info, mode) => {
    name = layer.label.split("|")[0]
    layerInfo = info.filter(i => i.name == name)[0]
    if (mode == "IR") {
        config = layerInfo.attribute
    ...
    s = '';
    // console.info(Object.keys(config))
    
    var key_words = Object.keys(config)
    // console.info(key_words.length)
    for(var i=0; i < key_words.length; i++) {
        
        val = config[key_words[i]]
        val_str = getAttr(val)
        // console.info(key_words[i])

        s += '<div class="node-param-row">';
        s += '<span class="node-param-key">' + key_words[i] + ': </span>';
        s += '<span class="node-param-value">' + val_str + '</span>';
        s += '</div>';
    }

    if ("definition" in layerInfo) {
        s += '<div class="node-param-row">';
        s += '<span class="node-param-key">' + "definition" + ': </span>';
        s += '<span class="node-param-value">' + layerInfo.definition + '</span>';
        s += '</div>';  
    }
...
const getInfoModel = (model) => {
    ...
    for (key in model) {
        ...
        s += '<div class="node-param-row">';
        s += '<span class="node-param-key">' + key + ': </span>';
        s += '<span class="node-param-value">' + val + '</span>';
        s += '</div>';
    }
...
const draw = (json) => {
    ...
    let nodeMasks = d3.selectAll('.node')
        ...
        .on("mouseover", function (d) {
            ...
            tooltip.html(layerAttr(d, info, mode))  
                .style("left", (d3.event.pageX + d.width * 0.9) + "px")  
                .style("top", (d3.event.pageY - nodeH) + "px")  
                .style("opacity",1.0);  
        })
...
const miniMap = (nodes, edges, width, height, miniScale) => {
    map = d3.select('#draw')
        .append("g")
        .attr("class", "miniMap")
        .attr("transform", `translate(${window.innerWidth * 0.6},0)`)
