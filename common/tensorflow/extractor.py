...
def _main():
    ...
    # The extractor only support models in 'input_layer_map.keys()'.
    parser.add_argument('-n', '--network', type=_text_type, help='Model Type', required=True,
        choices = input_layer_map.keys())

    parser.add_argument('-p', '--path',
        type=_text_type, help='Tensorflow Checkpoint file path', required=True)

    parser.add_argument('-o', '--output_path',
        type=_text_type, help='The Output Path')
    ...
    with tf.Session() as sess:
        ...
        save_path = saver.save(sess, os.path.join(args.output_path, "graph_{}.ckpt".format(file_name)))
