...
class PytorchEmitter(Recover):

    dtype_map = {
        model_pb2.FLOAT16 : "torch.float16",
        model_pb2.FLOAT32 : "torch.float32",
        model_pb2.FLOAT64 : "torch.float64",
        model_pb2.INT16 : "torch.int16",
        model_pb2.INT32 : "torch.int32",
        model_pb2.INT64 : "torch.int64",
        model_pb2.UINT8 : "torch.uint8",
        model_pb2.UINT16 : "torch.uint16"
    }
...
""")
    ...
    def emit_Pool(self, IR_node):
        ...
        if IR_node.get_attr('pooling_type').s == b"MAX":
            pool_name = "max_pool{}d".format(dim)
            # exstr = ", value=float('-Inf')"
        elif IR_node.get_attr('pooling_type').s == b"AVG":
            pool_name = "avg_pool{}d".format(dim)
            # exstr = ""
        else:
            raise ValueError()

        if IR_node.layer.attribute['global_pooling'].val.b:
    ...
    def emit_Dropout(self, IR_node):
        self.add_body(2, "{:<15} = F.dropout(input = {}, p = {}, training = self.training, inplace = True)".format(
            IR_node.variable_name,
            self.parent_variable_name(IR_node),
            IR_node.layer.attribute["keep_prob"].f))
    ...
    def check_if_need_transpose(self, IR_node):
        ...
        dim = len(parent.layer.attribute['_output_shapes'].list.shape[0].dim)
        if dim > 2:
            original_dims = self.weights_dict[IR_node.name]['weights'].shape
            dims = [i.size for i in parent.layer.attribute['_output_shapes'].list.shape[0].dim[1:]] + [-1]
    ...
    def emit_FullyConnected(self, IR_node):
        ...
        in_features = 1
        for i in self.IR_graph.get_parent(IR_node.name, [0]).layer.attribute['_output_shapes'].list.shape[0].dim[1:]:
            in_features *= i.size
        ...
        self.add_init(2, "self.{} = self.__dense(name = '{}', in_features = {}, out_features = {}, bias = {})".format(
            ...
            IR_node.IR_layer.attribute["use_bias"].val.b))
    ...
    def emit_RNNs(self, IR_node, func):
        ...
        if "dropout" in IR_node.IR_layer.attr:
            dropout_str = ",dropout = {}, recurrent_dropout = {}".format(
                    IR_node.IR_layer.attribute['dropout'].f,
                    IR_node.IR_layer.attribute['recurrent_dropout'].f)
        else:
            dropout_str = ""
        ...
        code = "{:<15} = {}(units = {}, use_bias = {} {})({})".format(
                IR_node.name,
                func,
                IR_node.IR_layer.attribute['units'].i,
                IR_node.IR_layer.attribute['use_bias'].b,
                dropout_str,
                IR_node.in_edges[0])
    ...
    def emit_BatchNorm(self, IR_node):
        ...
        dim = len(IR_node.layer.attribute['_output_shapes'].list.shape[0].dim) - 2

        output_shape = IR_node.layer.attribute['_output_shapes'].list.shape[0]
        ...
        self.add_init(2, "self.{} = self.__batch_normalization({}, '{}', num_features={}, eps={}, momentum={})".format(
             ...
             IR_node.name,
             num_features,
             IR_node.layer.attribute['epsilon'].val.f,
             IR_node.layer.attribute['momentum'].val.f,
        ))
    ...
    def emit_Scale(self, IR_node):
        ...
        self.add_init(2, "self.{} = self.__scale({}, '{}', num_features={})".format(
             ...
             dim,
             IR_node.name,
             IR_node.layer.attribute['_output_shapes'].list.shape[0].dim[-1].size
        ))
    ...
    def emit_Const(self, IR_node):
        if 'dtype' in IR_node.layer.attr:
            dtype_str = "dtype={}".format(self.dtype_map[IR_node.layer.attribute['dtype'].type])
            if 'int' in dtype_str:
                self.add_body(2, "{:<15} = torch.tensor({}, {})".format(
                    IR_node.variable_name,
                    IR_node.layer.attribute['value'].i,
                    dtype_str))
            else:
                self.add_body(2, "{:<15} = torch.tensor({}, {})".format(
                    IR_node.variable_name,
                    IR_node.layer.attribute['value'].f,
                    dtype_str))
