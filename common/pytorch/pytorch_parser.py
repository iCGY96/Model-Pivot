...
from common.pytorch.pytorch_graph import PytorchGraph
import torch
import torchvision

class PytorchParser(Converter):
    ...
    def _set_output_shape(self, source_node, IR_node):
        ...
        IR_node.attribute["_output_shapes"].list.shape.extend([shape])
    ...
    def rename_BatchNormalization(self, source_node):
        ...
        # epsilon
        IR_node.attribute['epsilon'].val.f = attr['epsilon']
    ...
    def rename_FullyConnected(self, source_node):
        ...
        if bias_name in self.state_dict:
            ...
            IR_node.attribute['use_bias'].val.b = False

        # units
        IR_node.attribute['units'].val.i = output_channels


    def rename_Dropout(self, source_node):
        IR_node = self._convert_identity_operation(source_node, new_op='Dropout')
        IR_node.attribute['keep_prob'].val.f = source_node.attrs['ratio']
    ...
    def rename_Concat(self, source_node):
        ...
        if source_node.attrs['axis'] == 1:
            IR_node.attribute['axis'].val.i = len(self.shape_dict[source_node.name]) - 1
